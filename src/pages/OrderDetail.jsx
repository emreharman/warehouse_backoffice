  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700">
        <div><strong>Sipariş No:</strong> #{order._id}</div>
        <div><strong>Tarih:</strong> {new Date(order.createdAt).toLocaleDateString("tr-TR")}</div>
        <div><strong>Durum:</strong> {statusLabels[order.status] || order.status}</div>
        <div><strong>Toplam Tutar:</strong> {order.totalPrice || 0} ₺</div>
        <div><strong>Tip:</strong> {order.type}</div>
        <div><strong>Not:</strong> {order.note || "-"}</div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <p><strong>İsim:</strong> {order.customer?.name}</p>
          ...
          order.items.map((item, index) => (
            ...
            >
              <div className="grid sm:grid-cols-2 gap-4 text-gray-700">
                <div><strong>Ürün Tipi:</strong> {item.productType}</div>
                <div><strong>Adet:</strong> {item.quantity}</div>
                <div><strong>Renk:</strong> {item.selectedVariant?.color}</div>
                <div><strong>Beden:</strong> {item.selectedVariant?.size}</div>
                <div><strong>Kumaş Kalitesi:</strong> {item.selectedVariant?.quality}</div>
                <div><strong>Kalıp:</strong> {item.selectedVariant?.fit}</div>
                <div><strong>Fiyat:</strong> {item.selectedVariant?.price || 0} ₺</div>
                <div><strong>İndirim:</strong> {item.selectedVariant?.discount || 0} ₺</div>
              </div>

              {item.designMeta && (
                <div>
                  <h3 className="font-semibold text-gray-800 mb-2">Tasarım Detayı</h3>
                  <div className="text-gray-700 space-y-1">
                    <p><strong>Yüz:</strong> {item.designMeta.side}</p>
                    <p><strong>Boyut:</strong> {item.designMeta.size}</p>
                    <p><strong>Konum:</strong> {item.designMeta.position}</p>
                    <p><strong>Pixel Pozisyonu:</strong> x:{item.designMeta.pixelPosition?.x}, y:{item.designMeta.pixelPosition?.y}</p>
                    <p><strong>Dosya:</strong> {item.designMeta.fileName}</p>
                    {item.designMeta.finalDesign && (
                      <div className="flex flex-col items-start mt-3">
                        <img
                          src={item.designMeta.finalDesign}
                          alt="Final Design"
                          onClick={() => setModalImage(item.designMeta.finalDesign)}
              ...
              {item.designFiles?.length > 0 && (
                <div className="mt-4">
                  <h3 className="font-semibold text-gray-800 mb-2">Üyklenen Dosyalar:</h3>
